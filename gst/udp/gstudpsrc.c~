...
gst_udpsrc_class_init (GstUDPSrcClass * klass)
{
 ...
 // vvv wenfeng
  g_object_class_install_property (G_OBJECT_CLASS (klass), KEEP_ALIVE_TIME,
      g_param_spec_uint64 ("keep-alive-time", "Keep-alive-time",
          "send keep alive packet interval time (second)", 0,
          G_MAXUINT64, 0,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, KEEP_ALIVE_STR,
      g_param_spec_string ("keep-alive-string", "Keep-alive-string",
          "keep alive string content", "helloword",
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (G_OBJECT_CLASS (klass), KEEP_ALIVE_LEN,
      g_param_spec_uint64 ("keep-alive-len", "Keep-alive-len",
          "send keep alive packet len", 0,
          G_MAXUINT64, 9,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

 g_object_class_install_property (gobject_class, NAT_FLAG,
      g_param_spec_boolean ("nat-flag", "Nat-flag",
          "udpsr if is nat behind",
          UDP_DEFAULT_CLOSE_SOCKET,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, NAT_ADDRESS,
      g_param_spec_string ("nat-address", "Nat-address",
          "send nat packet to src ip", UDP_DEFAULT_MULTICAST_GROUP,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (G_OBJECT_CLASS (klass), NAT_PORT,
      g_param_spec_int ("nat-port", "Nat-port",
          "send nat packet to src ip port", 0, G_MAXUINT16,
          UDP_DEFAULT_PORT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_udpsrc_finalize (GObject * object)
{
  ...
  if(udpsrc->time_id != 0)
  g_source_remove(udpsrc->time_id); // wenfeng
	udpsrc->time_id = 0;
	//printf("udpsrc->time_id %d \n", udpsrc->time_id);
 ...
 // udpsrc->keep_alive_str = NULL;

   g_free (udpsrc->nat_address);
  udpsrc->nat_address = NULL;
...
gst_udpsrc_free_cancellable (GstUDPSrc * src)
{
 ...
 gboolean senddatacb(gpointer udpsrc1)
 {
	GstUDPSrc *udpsrc;
	udpsrc = (GstUDPSrc *)udpsrc1;
  
	
	GError *err = NULL;
	//printf("-===%p \n", udpsrc1);
	if(udpsrc->used_socket == NULL)
	{
	//	printf("udpsrc->socket is null \n");
		udpsrc->time_id = 0;
		return FALSE;
	}
	else
...
retry:
  ...
  res =
	...
	// vvv wenfeng
	//udpsrc->saddr = *p_saddr;
    if(res> 0 && udpsrc->time_id == 0 &&  udpsrc->saddr != NULL && udpsrc->keep_alive_time !=0)
	{
        gst_element_post_message (GST_ELEMENT_CAST (udpsrc),
            gst_message_new_element (GST_OBJECT_CAST (udpsrc),
                gst_structure_new ("GstUDPSrcReceivedata",
                    "gotdata", G_TYPE_UINT64, res, NULL)));

		udpsrc->time_id = g_timeout_add_seconds(udpsrc->keep_alive_time,senddatacb,udpsrc);
		//printf("----===%p \n", udpsrc);
	}
   // printf("udpsrc->time_id = %d \n", udpsrc->time_id);
	// ^^^ wenfeng
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
	...
	case KEEP_ALIVE_TIME:
		udpsrc->keep_alive_time =  g_value_get_uint64 (value);
	  	break;
	case KEEP_ALIVE_LEN:
		udpsrc->keep_alive_len =  g_value_get_uint64 (value);
		//printf("len------%d \n",udpsrc->keep_alive_len);
	 	break;
	case KEEP_ALIVE_STR:
	{
		const gchar *group1;
	...
	case NAT_FLAG:
		udpsrc->nat_flag = g_value_get_boolean (value);
		break;
	case NAT_ADDRESS:
		{
      const gchar *group;
      ...
      g_free (udpsrc->nat_address);
      if ((group = g_value_get_string (value)))
        udpsrc->nat_address = g_strdup (group);
      else
        udpsrc->nat_address = g_strdup (UDP_DEFAULT_MULTICAST_GROUP);
      break;
       }
	...
	case NAT_PORT:
		udpsrc->nat_port = g_value_get_int (value);
		break;
    default:
      break;
  }
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
	...
	case KEEP_ALIVE_TIME:
		//udpsrc->keep_alive_time =  g_value_get_uint64 (value);
		g_value_set_uint64 (value, udpsrc->keep_alive_time);
	  	break;
	case KEEP_ALIVE_LEN:
		//udpsrc->keep_alive_len =  g_value_get_uint64 (value);
		g_value_set_uint64 (value, udpsrc->keep_alive_len);
	 	break;
	case KEEP_ALIVE_STR:
 		 g_value_set_string (value, udpsrc->keep_alive_str);
	 	break;

     case NAT_FLAG:
		 g_value_set_boolean (value, udpsrc->nat_flag);
		break;
	case NAT_ADDRESS:
		 g_value_set_string (value, udpsrc->nat_address);
         break;
	case NAT_PORT:
		g_value_set_int (value, udpsrc->nat_port);
		break;
...
#pragma message("No API available for getting IPv6 destination address")
 ...
 // vvv wenfeng
  if(src->nat_flag == 1)
  {
  	...
  	dst_address= G_SOCKET_ADDRESS(g_inet_socket_address_new(g_inet_address_new_from_string(src->nat_address) ,src->nat_port));

	g_socket_send_to (src->used_socket,dst_address,
		"helloworld", 9,  NULL, &err);
